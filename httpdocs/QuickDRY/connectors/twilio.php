...
class Twilio
{
    ...
    {
        /* @var $user ms_FinancialsAPPUsersClass */
        $user = ms_FinancialsAPPUsersClass::Get(['U_MOBILE'=>$phone_number]);
        $tl = new TbLogsTwilioLogClass();
        if($user) {
            $tl->user_id = $user->U_ID;
        }
    ...
    {
        if($allow || TbLogsPhoneLogClass::CheckNumber($mobile_number)) {
